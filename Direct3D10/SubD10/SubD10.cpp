                                      ...
                                      void* pUserContext )
{
    ...
    for( UINT i = 0; i < g_iNumSubDMeshes; i++ )
    {
        ...
        if( g_MeshDesc[i].m_szHeightMap[0] )
...
void ConvertFromSubDToBezier( ID3D10Device* pd3dDevice, CSubDMesh* pMesh )
{
    ...
    for( int iPassIndex = 0; iPassIndex < NumPasses; iPassIndex++ )
    {
        ...
        ID3D10Buffer* pSubBuffers[1];
        pSubBuffers[0] = pMesh->GetSubDPatchVB();
        ...
        if( iPassIndex == 1 )
        {
            ...
            pSubBuffers[0] = pMesh->GetSubDPatchRegVB();
        }
        ...
        // Set patch vb
        pd3dDevice->IASetVertexBuffers( 0, 1, pSubBuffers, stride, zeroOffset );

        // Stream to BezierB buffer
        pSubBuffers[0] = pMesh->GetPatchesBufferB();
        pd3dDevice->SOSetTargets( 1, pSubBuffers, offset );
        pTechB->GetDesc( &Desc );
        ...
        if( iPassIndex != 1 )
        {
                ...
                g_pTechCurrent == g_pTechRenderSceneBumpTan )
            {
                ...
                pSubBuffers[0] = pMesh->GetPatchesBufferUV();
                pd3dDevice->SOSetTargets( 1, pSubBuffers, offset );
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    for( UINT i = 0; i < g_iNumSubDMeshes; i++ )
    {
        ...
        if( pRenderTechnique == g_pTechRenderSceneBump && g_MeshDesc[i].m_szHeightMap[0] )
        pRenderTechnique = g_pTechRenderScene;
        else if( pRenderTechnique == g_pTechRenderSceneBumpTan && g_MeshDesc[i].m_szHeightMap[0] )
