...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
        ...
        {
            D3DXMATRIXA16 world = *g_ArcBall[i].GetRotationMatrix() * *g_ArcBall[i].GetTranslationMatrix();
            world = g_amWorld[i] * world;
            D3DXMATRIXA16 mRot;
            D3DXMATRIXA16 mTrans;
            D3DXMatrixTranslation( &mTrans, 0.0f, 0.0f, -3.0f );
            D3DXMatrixRotationY( &mRot, fAngleDelta * ( i - g_nActiveMesh ) );
            world *= mTrans * mRot * g_mScroll;
            mWorldView = world * *g_Camera.GetViewMatrix();
            mWorldViewProjection = world * mViewProj;
            V( g_pEffect->SetMatrix( "g_mWorld", &world ) );
