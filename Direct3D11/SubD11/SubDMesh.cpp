...
HRESULT CSubDMesh::LoadSubDFromSDKMesh(ID3D11Device* pd3dDevice, const WCHAR* strFileName, const WCHAR* strAnimationFileName, const CHAR* strCameraName)
{
    ...
    for (UINT i = 0; i < MeshCount; ++i)
    {
        ...
        else
        {
            ...
            for (INT iPatch = 0; iPatch < iNumPatches; ++iPatch)
            {
                // How many regular patches do we have?
                if (pPatchData[iPatch].val[0] == 4 && pPatchData[iPatch].val[1] == 4 && pPatchData[iPatch].val[2] == 4 && pPatchData[iPatch].val[3] == 4)
                    ++nRegular;

                // What's the highest and lowest valence?
                for (INT j = 0; j < 4; ++j)
                {
                    if (pPatchData[iPatch].val[j] > nHighestVal)
                        nHighestVal = pPatchData[iPatch].val[0];

                    if (pPatchData[iPatch].val[j] < nLowestVal)
                        nLowestVal = pPatchData[iPatch].val[j];
                }
                ...
                if ((pPatchData[iPatch].val[0] != 4 || pPatchData[iPatch].val[1] != 4 || pPatchData[iPatch].val[2] != 4 || pPatchData[iPatch].val[3] != 4) &&
                    pPatchPiece->m_iRegularExtraodinarySplitPoint == -1)
                {
                    pPatchPiece->m_iRegularExtraodinarySplitPoint = iPatch;
                }
                ...
                {
                    assert(pPatchData[iPatch].val[0] != 4 || pPatchData[iPatch].val[1] != 4 || pPatchData[iPatch].val[2] != 4 || pPatchData[iPatch].val[3] != 4);
                }
            }
            ...
            for (UINT iSub = 0; iSub < nNumSub; ++iSub) // loop through all subsets inside this patch piece
            {
                SDKMESH_SUBSET* pSubset = m_pMeshFile->GetSubset(pPatchPiece->m_MeshIndex, iSub);
                ...
                pPatchPiece->RegularPatchStart.push_back(static_cast<UINT>(vRegularPatchData.size()));
                pPatchPiece->ExtraordinaryPatchStart.push_back(static_cast<UINT>(vExtraordinaryPatchData.size()));
                ...
                pPatchPiece->RegularPatchCount.push_back(
                    static_cast<UINT>(vRegularPatchData.size() - pPatchPiece->RegularPatchStart[pPatchPiece->RegularPatchStart.size() - 1]));
                pPatchPiece->ExtraordinaryPatchCount.push_back(
                    static_cast<UINT>(vExtraordinaryPatchData.size() - pPatchPiece->ExtraordinaryPatchStart[pPatchPiece->ExtraordinaryPatchStart.size() - 1]));
            }
            ...
            D3D11_BUFFER_DESC desc;
            desc.ByteWidth = static_cast<UINT>(vRegularIdxBuf.size()) * sizeof(UINT);
            ...
            desc.ByteWidth = static_cast<UINT>(vExtraordinaryIdxBuf.size()) * sizeof(UINT);
            ...
            desc.ByteWidth = static_cast<UINT>(vRegularPatchData.size()) * sizeof(PatchData);
            ...
            desc.ByteWidth = static_cast<UINT>(vExtraordinaryPatchData.size()) * sizeof(PatchData);
            ...
            SRVDesc.Buffer.NumElements = static_cast<UINT>(vRegularPatchData.size()) * 2;
            ...
            SRVDesc.Buffer.NumElements = static_cast<UINT>(vExtraordinaryPatchData.size()) * 2;
...
//--------------------------------------------------------------------------------------
void CSubDMesh::Update(const XMMATRIX& pWorld, double fTime)
{
...
bool CSubDMesh::GetCameraViewMatrix(XMMATRIX& pViewMatrix, XMVECTOR& pCameraPosWorld)
{
    ...
    const XMMATRIX& pCameraWorld = m_pMeshFile->GetWorldMatrix(m_iCameraFrameIndex);
    XMFLOAT4X4A cameraWorld;
    XMStoreFloat4x4A(&cameraWorld, pCameraWorld);
    pCameraPosWorld = XMVectorSet(cameraWorld._41, cameraWorld._42, cameraWorld._43, 0);
...
//--------------------------------------------------------------------------------------
void ExpandAABB(XMVECTOR& vCenter, XMVECTOR& vExtents, const XMVECTOR& vAddCenter, const XMVECTOR& vAddExtents)
{
    if (XMVectorGetX(vExtents) == 0 && XMVectorGetY(vExtents) == 0 && XMVectorGetZ(vExtents) == 0)
    {
        vCenter = vAddCenter;
    ...
    XMVECTOR vCurrentMin = vCenter - vExtents;
    XMVECTOR vCurrentMax = vCenter + vExtents;

    const XMVECTOR vAddMin = vAddCenter - vAddExtents;
    const XMVECTOR vAddMax = vAddCenter + vAddExtents;

    vCurrentMax = XMVectorMax(vCurrentMax, vAddMax);
    vCurrentMin = XMVectorMin(vCurrentMin, vAddMin);
...
//--------------------------------------------------------------------------------------
void CSubDMesh::GetBounds(XMFLOAT3* pvCenter, XMFLOAT3* pvExtents) const
{
    ...
    INT iCount = static_cast<INT>(GetNumPatchPieces());
    for (INT i = 0; i < iCount; ++i)
    {
        ...
        vPieceCenter = XMVector3TransformCoord(XMLoadFloat3(&Piece.m_vCenter), matTransform);
        vPieceExtents = XMVector3TransformNormal(XMLoadFloat3(&Piece.m_vExtents), matTransform);
        ExpandAABB(vCenter, vExtents, vPieceCenter, vPieceExtents);
    }

    iCount = static_cast<INT>(GetNumPolyMeshPieces());
    for (INT i = 0; i < iCount; ++i)
    {
    ...
    XMStoreFloat3(pvCenter, vCenter);
    XMStoreFloat3(pvExtents, vExtents);
}
...
{
    int PieceCount = static_cast<INT>(GetNumPatchPieces());
    ...
    PieceCount = static_cast<INT>(GetNumPolyMeshPieces());
    for (int i = 0; i < PieceCount; ++i)
    {
