...
{
    XMFLOAT4X4A mConstBoneWorld[MAX_BONE_MATRICES];
};

struct CB_PER_FRAME_CONSTANTS
{
    XMFLOAT4X4A mViewProjection;
        ...
        if (strCommand[0] == L'-' || strCommand[0] == L'/')
        {
            ...
            else if (wcsstr(strCommand, L"msaa") == strCommand)
            {
                ...
                if (strCommand != NULL)
                {
                    ...
                    g_iMSAASampleCount = std::min(16U, std::max(1U, g_iMSAASampleCount));
                }
            }
    ...
    void* /*pUserContext*/)
{
    ...
    XMFLOAT3 vCenter, vExtents;
    g_SubDMesh.GetBounds(&vCenter, &vExtents);
    XMFLOAT3 vEye;
    if (g_bCloseupCamera)
    {
        FLOAT fRadius = Length(vExtents);
    ...
    g_Camera.SetViewParams(XMLoadFloat3(&vEye), XMLoadFloat3(&vCenter));
...
void RenderSubDMesh(ID3D11DeviceContext* pd3dDeviceContext, CSubDMesh* pMesh, ID3D11PixelShader* pPixelShader)
{
    ...
    int PieceCount = static_cast<int>(pMesh->GetNumPatchPieces());
    ...
    for (int i = 0; i < PieceCount; ++i)
    {
        ...
        for (int j = 0; j < NumTransforms; ++j)
        {
            ...
            {
                XMStoreFloat4x4A(&pData->mConstBoneWorld[j], XMMatrixIdentity());
            }
            else
            {
                XMStoreFloat4x4A(&pData->mConstBoneWorld[j], XMMatrixTranspose(pMesh->GetInfluenceMatrix(MeshIndex, j)));
            }
        }
        if (NumTransforms == 0)
        {
            ...
            XMStoreFloat4x4A(&pData->mConstBoneWorld[0], XMMatrixTranspose(matTransform));
        }
    ...
    PieceCount = static_cast<int>(pMesh->GetNumPolyMeshPieces());
    for (INT i = 0; i < PieceCount; ++i)
    {
        // Per frame cb update
        D3D11_MAPPED_SUBRESOURCE MappedResource;
    ...
    float fElapsedTime, void* /*pUserContext*/)
{
    ...
    XMStoreFloat4x4A(&pData->mViewProjection, XMMatrixTranspose(mViewProjection));
