...
HRESULT VarianceShadowsManager::ReleaseAndAllocateNewShadowResources(ID3D11Device* pd3dDevice)
{
        ...
        || m_CopyOfCascadeConfig.m_iBufferSize != m_pCascadeConfig->m_iBufferSize)
    {
        ...
        D3D11_TEXTURE2D_DESC dtd = {
            static_cast<UINT>(m_CopyOfCascadeConfig.m_iBufferSize),//UINT Width;
            static_cast<UINT>(m_CopyOfCascadeConfig.m_iBufferSize),//UINT Height;
            1,//UINT MipLevels;
            static_cast<UINT>(m_CopyOfCascadeConfig.m_nCascadeLevels),//UINT ArraySize;
...
HRESULT VarianceShadowsManager::InitFrame(ID3D11Device* pd3dDevice, CDXUTSDKMesh* /*mesh*/)
{
    ...
    const XMMATRIX &mD3DXViewCameraProjection = m_pViewerCamera->GetProjMatrix();
    XMMATRIX matViewCameraProjection = mD3DXViewCameraProjection;
    const XMMATRIX &mD3DXViewCameraView = m_pViewerCamera->GetViewMatrix();
    XMMATRIX matViewCameraView = mD3DXViewCameraView;
    const XMMATRIX &mD3DLightView = m_pLightCamera->GetViewMatrix();
    XMMATRIX matLightCameraView = mD3DLightView;
    ...
    for (INT iCascadeIndex = 0; iCascadeIndex < m_CopyOfCascadeConfig.m_nCascadeLevels; ++iCascadeIndex)
    {
        ...
        //These are the un-configured near and far plane values.  They are purposely awful to show 
...
HRESULT VarianceShadowsManager::RenderShadowsForAllCascades(ID3D11Device* /*pd3dDevice*/,
    ...
    for (int iCurrentCascade = 0; iCurrentCascade < m_CopyOfCascadeConfig.m_nCascadeLevels; ++iCurrentCascade)
    {
        ...
        XMStoreFloat4x4A(&pVSPerObject->m_World, XMMatrixTranspose(id));
...
HRESULT VarianceShadowsManager::RenderScene(ID3D11DeviceContext* pd3dDeviceContext,
    ...
    XMMATRIX dxmatCameraProj = pActiveCamera->GetProjMatrix();
    XMMATRIX dxmatCameraView = pActiveCamera->GetViewMatrix();
    ...
    XMStoreFloat4x4A(&pcbAllShadowConstants->m_WorldViewProj, XMMatrixTranspose(dxmatWorldViewProjection));
    XMStoreFloat4x4A(&pcbAllShadowConstants->m_WorldView, XMMatrixTranspose(dxmatCameraView));
    ...
    XMStoreFloat4x4A(&pcbAllShadowConstants->m_World, XMMatrixTranspose(dxmatIdentity));
    XMMATRIX dxmatTextureScale = XMMatrixScaling(
        0.5f,
        -0.5f,
        1.0f);

    XMMATRIX dxmatTextureTranslation = XMMatrixTranslation(.5, .5, 0);
    XMMATRIX scaleToTile = XMMatrixScaling(1.0f / (float)m_pCascadeConfig->m_nCascadeLevels, 1.0, 1.0);

    XMStoreFloat4x4A(&pcbAllShadowConstants->m_Shadow, XMMatrixTranspose(m_matShadowView));
    for (int index = 0; index < m_CopyOfCascadeConfig.m_nCascadeLevels; ++index)
    {
        XMMATRIX mShadowTexture = m_matShadowProj[index] * dxmatTextureScale * dxmatTextureTranslation;
        XMFLOAT4X4A matrixShadowTexture;
        XMStoreFloat4x4A(&matrixShadowTexture, mShadowTexture);
        pcbAllShadowConstants->m_vCascadeScale[index].x = matrixShadowTexture._11;
        pcbAllShadowConstants->m_vCascadeScale[index].y = matrixShadowTexture._22;
        pcbAllShadowConstants->m_vCascadeScale[index].z = matrixShadowTexture._33;
        pcbAllShadowConstants->m_vCascadeScale[index].w = 1;

        pcbAllShadowConstants->m_vCascadeOffset[index].x = matrixShadowTexture._41;
        pcbAllShadowConstants->m_vCascadeOffset[index].y = matrixShadowTexture._42;
        pcbAllShadowConstants->m_vCascadeOffset[index].z = matrixShadowTexture._43;
    ...
    ep -= lp;
    ep = XMVector4Normalize(ep);

    pcbAllShadowConstants->m_vLightDir = XMFLOAT4(XMVectorGetX(ep), XMVectorGetY(ep), XMVectorGetZ(ep), 1.0f);
