...
XMMATRIX                  g_m4x4MeshMatrix[MESH_TYPE_MAX];
XMFLOAT3                 g_v3AdaptiveTessParams[MESH_TYPE_MAX];
...
// Constant buffer layout for transferring data to the PN-Triangles HLSL functions
struct CB_PNTRIANGLES
{
    XMFLOAT4X4A f4x4World;               // World matrix for object
    XMFLOAT4X4A f4x4ViewProjection;      // View * Projection matrix
    XMFLOAT4X4A f4x4WorldViewProjection; // World * View * Projection matrix  
    ...
    XMFLOAT4 f4ViewFrustumPlanes[4]; // View frustum planes
};
...
void NormalizePlane(XMFLOAT4* pPlaneEquation);
void ExtractPlanesFromFrustum(XMFLOAT4* pPlaneEquation, const XMMATRIX* pMatrix);
    ...
    void* /*pUserContext*/)
{
    ...
    XMMATRIX mModelRotationX;
    XMMATRIX mModelRotationY;
    XMMATRIX mModelTranslation;
    // Tiny
    mModelRotationX = XMMatrixRotationX(-XM_PI / 2);
    mModelRotationY = XMMatrixRotationY(XM_PI);
    ...
    // Teapot
    g_m4x4MeshMatrix[MESH_TYPE_TEAPOT] = XMMatrixIdentity();
    // User
    mModelTranslation = XMMatrixTranslation(0.0f, -1.0f, 0.0f);
    g_m4x4MeshMatrix[MESH_TYPE_USER] = mModelTranslation;

    // Setup the light camera
    vecEye = XMVectorSet(0.0f, -1.0f, 1.0f, 0);
    ...
    {
        V_RETURN(CreateDDSTextureFromFile(pd3dDevice, L"media\\user\\diffuse.dds", NULL, &g_pDiffuseTextureSRV))
    ...
    const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* /*pUserContext*/)
{
    ...
    {
        g_Camera[iMeshType].SetProjParams(XM_PI / 4, fAspectRatio, 1.0f, 5000.0f);
    ...
    g_LightCamera.SetProjParams(XM_PI / 4, fAspectRatio, 1.0f, 5000.0f);
    ...
    float fElapsedTime, void* /*pUserContext*/)
{
    ...
    XMVECTOR v3LightDir = g_LightCamera.GetEyePt() - g_LightCamera.GetLookAtPt();
    v3LightDir = XMVector3Normalize(v3LightDir);

    // Get the view vector.
    XMVECTOR v3ViewVector = g_Camera[g_eMeshType].GetEyePt() - g_Camera[g_eMeshType].GetLookAtPt();
    v3ViewVector = XMVector3Normalize(v3ViewVector);

    // Calculate the plane equations of the frustum in world space
    XMFLOAT4 f4ViewFrustumPlanes[6];
    ...
    XMStoreFloat4x4A(&pPNTrianglesCB->f4x4World, XMMatrixTranspose(mWorld));
    XMStoreFloat4x4A(&pPNTrianglesCB->f4x4ViewProjection, XMMatrixTranspose(mViewProjection));
    XMStoreFloat4x4A(&pPNTrianglesCB->f4x4WorldViewProjection, XMMatrixTranspose(mWorldViewProjection));
    pPNTrianglesCB->fLightDir[0] = XMVectorGetX(v3LightDir);
    pPNTrianglesCB->fLightDir[1] = XMVectorGetY(v3LightDir);
    pPNTrianglesCB->fLightDir[2] = XMVectorGetZ(v3LightDir);
    pPNTrianglesCB->fLightDir[3] = 0.0f;
    pPNTrianglesCB->fEye[0] = XMVectorGetX(g_Camera[g_eMeshType].GetEyePt());
    pPNTrianglesCB->fEye[1] = XMVectorGetY(g_Camera[g_eMeshType].GetEyePt());
    pPNTrianglesCB->fEye[2] = XMVectorGetZ(g_Camera[g_eMeshType].GetEyePt());
    pPNTrianglesCB->fViewVector[0] = XMVectorGetX(v3ViewVector);
    pPNTrianglesCB->fViewVector[1] = XMVectorGetY(v3ViewVector);
    pPNTrianglesCB->fViewVector[2] = XMVectorGetZ(v3ViewVector);
...
void CaptureFrame()
{
    ...
    if (TexDesc.SampleDesc.Count > 1)
    {
        ...
        SaveToPNGFile(DXUTGetD3D11DeviceContext(), g_pCaptureTexture, g_CmdLineParams.strCaptureFilename);
...
//--------------------------------------------------------------------------------------
void ExtractPlanesFromFrustum(XMFLOAT4* pPlaneEquation, const XMMATRIX* xmmatrix)
{
    XMFLOAT4X4A matrix;
    XMStoreFloat4x4A(&matrix, *xmmatrix);
    XMFLOAT4X4A *pMatrix = &matrix;
