...
static void GetSurfaceInfo( UINT width, UINT height, DXGI_FORMAT fmt, UINT* pNumBytes, UINT* pRowBytes, UINT* pNumRows )
{
    ...
    if( bc )
    {
        int numBlocksWide = 0;
        if( width > 0 )
            numBlocksWide = std::max( 1UL, width / 4UL );
        int numBlocksHigh = 0;
        if( height > 0 )
            numBlocksHigh = std::max( 1UL, height / 4UL );
                                     ...
                                     UINT BitSize, __out ID3D11ShaderResourceView** ppSRV, bool bSRGB )
{
    ...
    else
    {
        ...
        if( desc.Format == DXGI_FORMAT_UNKNOWN )
        {
            ...
            {
                if (pHeader->ddspf.dwRBitMask == 0x00ff0000
                    && pHeader->ddspf.dwGBitMask == 0x0000ff00
                    && pHeader->ddspf.dwBBitMask == 0x000000ff)
                {
                    // format is XRGB or ARGB
