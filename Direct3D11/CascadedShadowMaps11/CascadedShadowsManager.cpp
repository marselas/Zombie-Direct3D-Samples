...
HRESULT CascadedShadowsManager::ReleaseAndAllocateNewShadowResources(ID3D11Device* pd3dDevice)
{
        ...
        || m_CopyOfCascadeConfig.m_iBufferSize != m_pCascadeConfig->m_iBufferSize)
    {
        ...
        {
            static_cast<UINT>(m_CopyOfCascadeConfig.m_iBufferSize * m_CopyOfCascadeConfig.m_nCascadeLevels),//UINT Width;
...
HRESULT CascadedShadowsManager::InitFrame(ID3D11Device* pd3dDevice, CDXUTSDKMesh* /*mesh */)
{
    ...
    // Copy D3DX matrices into XNA Math matrices.
...
HRESULT CascadedShadowsManager::RenderShadowsForAllCascades(ID3D11Device* /*pd3dDevice*/,
    ...
    for (INT currentCascade = 0; currentCascade < m_CopyOfCascadeConfig.m_nCascadeLevels; ++currentCascade)
    {
        ...
        pcbAllShadowConstants->m_WorldViewProj = XMMatrixTranspose(dxmatWorldViewProjection);
        ...
        pcbAllShadowConstants->m_World = XMMatrixTranspose(matIdentity);
...
HRESULT CascadedShadowsManager::RenderScene(ID3D11DeviceContext* pd3dDeviceContext,
    ...
    CB_ALL_SHADOW_DATA* pcbAllShadowConstants = (CB_ALL_SHADOW_DATA*)MappedResource.pData;
    pcbAllShadowConstants->m_WorldViewProj = XMMatrixTranspose(dxmatWorldViewProjection);
    pcbAllShadowConstants->m_WorldView = XMMatrixTranspose(dxmatCameraView);
    ...
    pcbAllShadowConstants->m_World = XMMatrixTranspose(dxmatIdentity);
    XMMATRIX dxmatTextureScale = XMMatrixScaling(0.5f, -0.5f, 1.0f);
    ...
    for (int index = 0; index < m_CopyOfCascadeConfig.m_nCascadeLevels; ++index)
    {
        ...
        XMFLOAT4X4 matShadow;
        XMStoreFloat4x4(&matShadow, mShadowTexture);

        pcbAllShadowConstants->m_vCascadeScale[index].x = matShadow._11;
        pcbAllShadowConstants->m_vCascadeScale[index].y = matShadow._22;
        pcbAllShadowConstants->m_vCascadeScale[index].z = matShadow._33;
        pcbAllShadowConstants->m_vCascadeScale[index].w = 1;

        pcbAllShadowConstants->m_vCascadeOffset[index].x = matShadow._41;
        pcbAllShadowConstants->m_vCascadeOffset[index].y = matShadow._42;
        pcbAllShadowConstants->m_vCascadeOffset[index].z = matShadow._43;
