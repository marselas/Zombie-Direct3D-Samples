    ...
    void* /*pUserContext*/)
{
    ...
    {
        V_RETURN(CreateDDSTextureFromFile(pd3dDevice, DXGI_FORMAT_R32G32B32A32_FLOAT, strPath, NULL, &pCubeRV));
    ...
    XMVECTOR vecEye = XMVectorSet(0.0f, -10.5f, -3.0f, 0);
...
HRESULT MeasureLuminancePS11(ID3D11DeviceContext* pd3dImmediateContext)
{
    ...
    for (int i = NUM_TONEMAP_TEXTURES - 1; i > 0; i--)
    {
        ...
        ID3D11RenderTargetView* rtViews[1] = { pSurfDest };
        pd3dImmediateContext->OMSetRenderTargets(1, rtViews, NULL);

        ID3D11ShaderResourceView* rViews[1] = { pTexSrc };
        pd3dImmediateContext->PSSetShaderResources(0, 1, rViews);
...
HRESULT MeasureLuminanceCS11(ID3D11DeviceContext* pd3dImmediateContext, const DXGI_SURFACE_DESC* pBackBufferDesc)
{
    ...
    // First CS pass, reduce the render target texture into a 1D buffer
    {
        ...
        CB_CS cbCS = { static_cast<UINT>(dimx), static_cast<UINT>(dimy), pBackBufferDesc->Width, pBackBufferDesc->Height };
        ...
        if (!g_bCPUReduction)
        {
            ...
            if (nNumToReduce > 1)
            {
                for (;;)
                {
                    ...
                    CB_CS cbCS = { static_cast<UINT>(nNumToReduce), static_cast<UINT>(dim), 0, 0 };
                    RunComputeShader(pd3dImmediateContext,
                        g_pReduceToSingleCS,
                        1, aRViews,
                        g_pcbCS, &cbCS, sizeof(cbCS),
