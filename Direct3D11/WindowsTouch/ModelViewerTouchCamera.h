...
bool CModelViewerTouchCamera::HandleTouchMessages(HWND /*hWnd*/,
    ...
    static XMFLOAT2 vLastTouchInputs[2];
    static TouchCameraState touchCameraState = TC_INIT;
    static XMFLOAT2 vCurrentTouchInputs[2];
    ...
    // only looks at the first two inputs.  Some camera based touch monitors will loose one finger due to
    ...
    if (nInputs > 1) {
        ...
        if (ti.dwFlags & TOUCHEVENTF_MOVE)
        {
            ...
            XMFLOAT2 diff1 = vCurrentTouchInputs[0] - vCurrentTouchInputs[1];
            XMFLOAT2 diff2 = vLastTouchInputs[0] - vLastTouchInputs[1];
            ...
            if (touchCameraState == TC_SECOND_INPUT_RECEIVED) {
                XMFLOAT2 finger1delta = vCurrentTouchInputs[0] - vLastTouchInputs[0];
                XMFLOAT2 finger2delta = vCurrentTouchInputs[1] - vLastTouchInputs[1];
                XMFLOAT2 finger1delta_norm, finger2delta_norm;
                finger1delta_norm = Normalize(finger1delta);
                finger2delta_norm = Normalize(finger2delta);

                float inphase = Dot(finger1delta_norm, finger2delta_norm);
                ...
                // if the dot product is negative the two inputs are coming together or moving apart
